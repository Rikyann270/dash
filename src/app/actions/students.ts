...
export async function getStudentDetails(studentId: string) {
  ...
  const [attendance, invoices, results] = await Promise.all([
    ...
    supabase.from("results").select("*, assessments(title, total_marks, subjects(name))").eq("student_id", studentId),
  ]);
...
export async function createStudent(formData: FormData) {
  ...
  if (useFallback) {
    ...
    await serverSupabase.from("profiles").update({ phone }).eq("id", userId);
  }

  const { error: studentError } = await serverSupabase.from("students").insert([
    {
      profile_id: userId,
