...
async function upsertAttendanceNotification(supabase: any, studentId: string, sessionId: string, status: string, date: string) {
  try {
...
export async function submitAttendance(formData: FormData) {
  ...
  const { data: sessionData } = await supabase.from('lesson_sessions').select('status').eq('id', lessonSessionId).single();
